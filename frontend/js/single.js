...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        const response = await fetch(`https://edubridge-n4rs.onrender.com/api/courses/${courseId}`);
        if (!response.ok) {
...
async function enrollInCourse(courseId) {
    ...
    try {
        const response = await fetch('https://edubridge-n4rs.onrender.com/api/enroll', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
    ...
    try {
        ...
        enrollButton.addEventListener('click', async function (e) {
            ...
            try {
                const enrollResponse = await fetch('https://edubridge-n4rs.onrender.com/api/enroll', {
                    method: 'POST',
